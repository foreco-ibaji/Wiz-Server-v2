...
//  }
  ...
  @Operation(summary = "미션 목록 조회 API")
  @GetMapping("/api/v1/mission")
  public ResponseEntity<DataResponse<MissionDto>>  getMissions(
      @RequestParam(name = "kind", required = true, defaultValue = "WIZ") String kind,
      @RequestParam(name = "difficulty", required = false) String difficulty,
    ...
    List<MissionDetailDto> missions = missionService.findMissions(kind, difficulty, sessionUser.getUuid());
    MissionDto filteredMissions = missionService.filterAvailable(missions, available);
    return new ResponseEntity<>(
        DataResponse.of(HttpStatus.OK, "미션 목록 조회 성공", filteredMissions), HttpStatus.OK);
  }
